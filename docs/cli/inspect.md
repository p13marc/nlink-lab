...
If LAB is a path ending in `.nlz`, summarizes the archive (manifest + node/link/network counts) without extracting. Otherwise, behaves as before — runs against a deployed lab.

Usage: nlink-lab inspect <LAB>
