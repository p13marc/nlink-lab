...
By default, dumps the rendered topology as TOML/JSON to stdout or `--output FILE`. With `--archive`, produces a portable `.nlz` lab archive (tar.gz with manifest + topology + params + rendered + checksums) suitable for sharing repros.

Usage: nlink-lab export [OPTIONS] <LAB>
